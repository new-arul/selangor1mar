...
BOT_TOKEN = "8759966690:AAHXL6sXgfk__PbB7IxPdwDLzKCqTAYnv8I"
CHAT_ID = "7311495050"
...
async def worker_main():
    ...
    while True:
            # ==== CLEAN FINISHED CLIENT TASK ====
        for key, task in list(clients.items()):
            if task.done():
                del clients[key]
                print("[CLIENT CLEANED]", key)

        # ==== AUTO CLEAN TMP CLONE SESSION ====
        try:
            tmp_dir = tempfile.gettempdir()
            now = time.time()

            active_keys = set(clients.keys())

            for f in os.listdir(tmp_dir):
                if "_clone_" in f and f.endswith(".session"):

                    path = os.path.join(tmp_dir, f)

                    # skip kalau masih dipakai client aktif
                    if any(f.startswith(f"{key}_clone_") for key in active_keys):
                        continue

                    # hapus kalau lebih dari 10 menit
                    if now - os.path.getmtime(path) > 600:
                        try:
                            os.remove(path)
                            print("[TMP CLEANED]", path)
                        except:
                            pass

        except Exception as e:
            print("[TMP CLEAN ERROR]", e)
